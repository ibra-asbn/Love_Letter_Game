...
class LoveLetterRLEnv(AECEnv):
    ...
    - Contrainte bayésienne publique "Baron gagné → borne min_kept_card"
    ...
    def step(self, action):
        ...
        if target and (target not in self.agents or self.terminations.get(target, True)):
            target = None
        if target and target == agent and card_played != 5:
            target = None
        if target and target != agent and self._protected.get(target, False):
            target = None
        ...
        if card == 1 and target:
            ...
            if self._hands[agent] and self._hands[target]:
                ...
                if my_val > their_val:
                    self._eliminate(target)
                    self.rewards[agent] += 0.2
                    # Information publique : tout le monde voit la carte du
                    # mort, donc sait seulement que le survivant avait plus.
                    self._min_kept_card[agent] = min(9, their_val + 1)
                elif their_val > my_val:
                    self._eliminate(agent)
                    self.rewards[agent] -= 0.2
                    self._min_kept_card[target] = min(9, my_val + 1)
                else:
                    # Les deux joueurs impliqués ont comparé exactement leurs
                    # cartes. L'information reste privée entre eux.
                    self._known_cards[agent][target].fill(0.0)
                    self._known_cards[agent][target][their_val] = 1.0
                    self._known_cards[target][agent].fill(0.0)
                    self._known_cards[target][agent][my_val] = 1.0
            ...
            if self._hands[agent] and self._hands[target]:
                previous_known_agent = {
                    observer: self._known_cards[observer][agent].copy()
                    for observer in self.possible_agents
                }
                previous_known_target = {
                    observer: self._known_cards[observer][target].copy()
                    for observer in self.possible_agents
                }
                previous_min_agent = self._min_kept_card[agent]
                previous_min_target = self._min_kept_card[target]

                self._hands[agent], self._hands[target] = self._hands[target], self._hands[agent]

                # Les connaissances deja acquises par des observateurs suivent
                # la carte qui vient d'etre echangee.
                for a in self.possible_agents:
                    self._known_cards[a][agent] = previous_known_target[a]
                    self._known_cards[a][target] = previous_known_agent[a]
                ...
                self._turns_hand_unchanged[agent] = 0
                self._turns_hand_unchanged[target] = 0
                self._min_kept_card[agent] = previous_min_target
                self._min_kept_card[target] = previous_min_agent
    ...
    def observe(self, agent):
        ...
        # [0:10] Ma main.
        # Pendant le sous-etat Chancelier, la main est vide dans l'etat interne:
        # les cartes choisissables vivent dans _chancellor_pool. On expose ce
        # pool ici pour que l'agent puisse reellement choisir quoi garder.
        private_cards = (
            self._chancellor_pool
            if self._chancellor_pending and agent == self.agent_selection
            else self._hands.get(agent, [])
        )
        ...
        # Pendant le choix Chancelier, on reutilise ce bloc pour exposer le
        # pool ordonne. L'action Chancelier depend de l'index dans ce pool;
        # sans cet ordre, le choix exact est partiellement inobservable.
        if self._chancellor_pending and agent == self.agent_selection:
            for pos in range(self.MAX_DECK_POS_TRACKED):
                if pos < len(self._chancellor_pool):
                    obs[idx + self._chancellor_pool[pos]] = 1.0
                idx += 10
        else:
