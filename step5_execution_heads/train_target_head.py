"""Train a Step5 target-execution head for Priest/Baron/King."""
...
def load_records(path: Path, kind: str) -> list[dict]:
    ...
    return [record for record in payload if record.get("kind") == kind]
...
def build_rows(records: list[dict], args) -> tuple[dict, dict]:
    ...
    for group_id, record in enumerate(records):
        if "observation" not in record:
            raise ValueError("Dataset must include `observation`; rerun collect_execution_teacher.")
        ...
        group_summaries.append(
            {
                ...
                "model_target": action_target(model_action),
    ...
    if not observations:
        raise RuntimeError(f"No trainable target rows found for kind={args.kind}.")
    arrays = {
    ...
    summary = {
        "kind": args.kind,
        "label": KIND_TO_LABEL.get(args.kind, args.kind),
        "card": KIND_TO_CARD[args.kind],
...
def group_metrics(head: TargetExecutionHead, data: dict, row_idx: np.ndarray, margin: float, device) -> dict:
    head.eval()
    ...
    for group in np.unique(groups):
        ...
        model_pos = int(exact_model_positions[0]) if len(exact_model_positions) else 0
...
def train(data: dict, args, logger: ExperimentLogger):
    ...
    head = TargetExecutionHead(hidden_dim=args.hidden_dim, dropout=args.dropout).to(device)
    optimizer = torch.optim.AdamW(head.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    loader = DataLoader(TensorDataset(torch.as_tensor(train_idx, dtype=torch.long)), batch_size=args.batch_size, shuffle=True)
    history = []
    logger.write(
        f"Debut entrainement tete cible {args.kind}",
        expected="Apprendre les avantages relatifs aux choix de cible.",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (idx,) in loader:
            idx_np = idx.numpy()
            obs = torch.as_tensor(data["observations"][idx_np], dtype=torch.float32, device=device)
            feat = torch.as_tensor(data["features"][idx_np], dtype=torch.float32, device=device)
            target = torch.as_tensor(data["targets"][idx_np], dtype=torch.float32, device=device)
            weight = torch.as_tensor(data["weights"][idx_np], dtype=torch.float32, device=device)
            pred = head(obs, feat)
        ...
        history.append(row)
        logger.write(
            f"Epoch tete cible {args.kind} {epoch}/{args.epochs}",
            expected="Monter top1/override agreement sans sur-override.",
            actual=(
                f"loss={row['loss']:.4f}, val_top1={row['val']['top1_acc']:.3f}, "
                f"val_override={row['val']['pred_override_rate']:.3f}, val_mae={row['val']['mae']:.3f}"
            ),
            details=row,
        )
...
def main() -> None:
    ensure_dirs()
    parser = argparse.ArgumentParser(description="Train Step5 target execution head.")
    ...
    records = load_records(dataset_path, args.kind)
    data, dataset_summary = build_rows(records, args)
    logger.write(
        f"Dataset tete cible {args.kind} charge",
        expected="Utiliser les etats naturels labellises par oracle CRN.",
    ...
    payload = {
        "model_type": "step5_target_execution_head",
    ...
    logger.write(
        f"Fin entrainement tete cible {args.kind}",
        expected="Sauvegarder une tete rapide candidate a evaluer.",
